...
struct OpenList {
	...
	* from calling ma
	*/
	void insert_update(const State& state, const Heuristic& h, const Map& map) {
		...
		if (_state_handle_index.find(state) == _state_handle_index.end()) {
			...
			auto handle = _min_heap.push(
				StateFvalPair(state, cur_gval + h(state))
			);
		...
		// state IS in the openlist, update it.
			...
			assert(h(cur_state) == h(state)); // this very much needs to be true
			_min_heap.update(handle, StateFvalPair{state, map.get_gval(state) + h(state)});
		}
	};
	State pop() {
